...
function UpdateOrgao() {
  ...
  // token e roles precisam ser lidos só no cliente
  ...
  // Busca os dados só quando router estiver pronto
  useEffect(() => {
  ...
  // Evita renderizar antes de checar auth (corrige mismatch do SSR)
  ...
  return (
    <div className={styles.container}>
      ...
      <form className={styles.inputs_container}>
        <div className={styles.inputs_box}>
          ...
          <div className="row">
            ...
            <div className={`col ${styles.col}`}>
              <label className="form-label">
                Selecionar Área <span className={styles.obrigatorio}>*</span>
              </label>
              {areas.length > 0 && (
                <select
                  value={orgao.area?.[0]?.id || ""}
      ...
      {showAlert && (
        <Alert
          message="Informações do órgão editadas com sucesso!"
          show={showAlert}
          url={`/lapa/gerenciarOrgaos`}
        />
      )}
      {showErrorAlert && (
        <ErrorAlert
